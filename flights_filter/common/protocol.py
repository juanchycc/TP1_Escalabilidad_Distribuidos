...
class Serializer(BaseSerializer):
    ...
    def send_pkt(self, pkt, original_pkt):
        ...
        for flight in pkt:
            group = self._get_group(flight[2], self._num_groups)
    ...
    def _send_pkt(self, pkt, key, header, original_pkt):
        ...
        logging.debug(
            f'Termine de procesar | pkt: {original_pkt.get_pkt_number()}')
        # time.sleep(10)
        self._middleware.send(pkt, key)
        logging.debug(
            f'Ya mande a la siguiente etapa | pkt: {original_pkt.get_pkt_number()}')

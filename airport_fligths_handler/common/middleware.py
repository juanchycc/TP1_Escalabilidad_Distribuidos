...
class Middleware(BaseMiddleware):
    ...
    def _connect_in_exchange(self, flights_exchange, airports_exchange, queue, routing_key, id):
        ...
        result = channel.queue_declare(
            queue=queue + "_flights", durable=True)
        queue_name_flights = result.method.queue
        channel.queue_bind(
            exchange=flights_exchange, queue=queue + "_flights", routing_key=id)
        result = channel.queue_declare(
            queue=queue + "_airports", durable=True)
        queue_name_airports = result.method.queue
        channel.queue_bind(
            exchange=airports_exchange, queue=queue + "_airports", routing_key=routing_key)
